 ...
 * 
 */
var SINGLELEAD_ShowFineAnnotationSingle = function() {
	fineTune_graph.setAnnotations(tempAnnotations);
};
...
//	alert("CVRG Message: Point on " + p.name + " clicked at: " + p.xval + " milliseconds, " + p.yval + "microVolts,  open annotation popup.");
	...
	SINGLELEAD_ShowFineAnnotationSingle();
...
/** updates the selected point for a new annotation, based on the new selection in the fine tuning graph. **/
...
var updateFineTuneValues = function(onX, offX){
	...
	var onY = fineTune_graph.getValue(onX, col);
	var offY = fineTune_graph.getValue(offX, col);
...
/** ignores the changes in a fine tuning graph which just closed. **/
...
var SINGLELEAD_drawCallback = function(ecg_graph) {
	...
	try{
		var listDiv = WAVEFORM_getElementByIdEndsWith("div","list_div");
		if(listDiv != null){
			...
			alert("SINGLELEAD_drawCallback() cannot find 'list_div'");
		}

	}catch (error) {
		alert("SINGLELEAD_drawCallback() Caught: " + error.message);
	}
//	var bDots=false;
...
var fineTune_graph = "not a graph";
...
//					},
	...
	var newWidth = 600; 
	var newHeight= 300;
	fineTune_graph.resize(newWidth, newHeight);
	CVRG_setFineLabels(displayMinV2, displayMaxV2, yLabel);
};
 ...
 * @param point - the fine tuning point being rendered, 0 = start of interval, 1 = end of inverval.
 * @param bShowGraph - Boolean if false don't draw the graph after all.
 */
var renderSingleGraphFineTuner = function(centerPoint, termName, point, bShowGraph){
	//alert('renderSingleGraphFineTuner(' + centerPoint + '), showFineGraph: '+ bShowGraph + ', termName: ' + termName + '');
	isMultigraph=false;
	fineTuningPoint=point;
...
/** Draws the fine tuning dygraph of the data 5 mseconds before and after the new annotation interval.
 * @param startMS - specific time of the start of the interval
 * @param endMS - specific time of the end of the interval
 * @param bShowGraph - Boolean if false don't draw the graph after all.
 */
var renderSingleGraphFineFullAnnotation = function(startMS, endMS, bShowGraph){
	if(bShowGraph){
...
/** Centers and zooms the fineTune_graph Y axis so that the full range of voltages 
 * found in the time period indicated is visible, with 15% of the range as padding above and below. 
 **/
var centerFineScaleVoltage = function(startTime,endTime){
	...
	displayMinV2 = ext.dataMin-(deltaV*.15); // leave a 15% space at the bottom
	displayMaxV2 = ext.dataMax+(deltaV*.15); // leave a 15% space at the top

	fineTune_graph.updateOptions({
		valueRange: [displayMinV2, displayMaxV2]
	});
...
var getFineDataMinMax = function (startTime, endTime){
	...
	for(var row=calPointCount+2;row<fineTune_graph.numRows();row++){
		val = fineTune_graph.getValue(row,col);
		val1 = fineTune_graph.getValue(row,1);
		val2 = fineTune_graph.getValue(row,2);
		if (val<=startTime){
			startRow=row;
			endRow=row;
		};
	...
	col = 1; 
	var dataMin = fineTune_graph.getValue(startRow,col);// initialize 
	var dataMax = dataMin;
	var val = 0;
 ...
 * @returns void
 */
var CVRG_setFineLabels = function (dMinV, dMaxV, yLabel){
	var bDots = CVRG_bShowDots();
	var newTimeLabel = CVRG_getnewTimeLabel();
	fineTune_graph.updateOptions({
